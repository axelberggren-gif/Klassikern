...
function RaceCard({ icon, name, distance, targetTime, date, statLabel, statValue, sessionsCount, totalMinutes, progressPercent, color }: RaceCardProps) {
    ...
    <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{distance} · Mal: {targetTime} · {date}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        ...
        <div className="text-center">
          ...
          <p className="text-[10px] text-slate-400">Traningstid</p>
        </div>
        <div className="text-center">
          <p className="text-lg font-bold" style={{ color }}>{statValue}</p>
          <p className="text-[10px] text-slate-400">{statLabel}</p>
        </div>
      </div>
...
function DaysUntil({ date, label }: { date: string; label: string }) {
  ...
  return (
    <div className="text-center">
      <p className="text-2xl font-bold text-slate-50">{diffDays}</p>
      <p className="text-[10px] text-slate-400">dagar till {label}</p>
    </div>
  );
}
      ...
      <div className="flex flex-col gap-4 px-4 py-4">
        ...
        <div className="rounded-2xl bg-emerald-500/10 border border-emerald-500/30 p-5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-emerald-400">Vasaloppet</h3>
              <p className="text-xs text-emerald-400/70">90 km · Avklarat! ✅</p>
            </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="rounded-xl bg-green-500/10 border border-green-500/20 p-3">
            <DaysUntil date="2026-09-26" label="Lidingo" />
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-slate-900 border border-slate-700 p-5">
          <h3 className="text-sm font-semibold text-slate-200 mb-3">Totalt</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-lg font-bold text-slate-50">{stats.totalSessions}</p>
                <p className="text-xs text-slate-400">Pass totalt</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Traningstid</p>
              </div>
            </div>
          </div>
        </div>
